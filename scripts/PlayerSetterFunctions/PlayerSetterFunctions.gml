...
function player_resolve_angle()
{
	...
	if (player_ray_collision(tilemaps, -x_radius, y_radius + 1)) mask_edge |= 1;
	if (player_ray_collision(tilemaps, x_radius, y_radius + 1)) mask_edge |= 2;
	if (player_ray_collision(tilemaps, 0, y_radius + 1)) mask_edge |= 4;
	
	// Check at ramp edges
	if (not landed)
	{
		if (player_ray_collision(tilemaps, -x_radius, y_radius + y_tile_reach)) ramp_edge |= 1;
		if (player_ray_collision(tilemaps, x_radius, y_radius + y_tile_reach)) ramp_edge |= 2;
		if (player_ray_collision(tilemaps, 0, y_radius + y_tile_reach)) ramp_edge |= 4;
	}
